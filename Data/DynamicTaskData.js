...
export default class DynamicTaskData {
  ...
  */
  ...
  getActionData = async (reportID, itemData) => {
    ...
    baseAPI.post('Task/GetActivityActions/', requestActionModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async getDynamicTaskData(taskIds, ontaskdetailsuccess) {
    ...
    udatabase.addlog('save task detail function called with task ids' + JSON.stringify(taskIds));
    let commonData = CommonDataManager.getInstance();
    let userData = await commonData.getUserDetail();
    let clientDetail = await commonData.getClientDetail();
    ...
    const onSuccess = ({ data }) => {
      ...
      data.Result.DataList.forEach(element =>
        database.setupTaskDetailAsync(requestModel.ClientAppID, element.RowID, element.RowJSON)
      );
    ...
    const onFailure = error => {
      udatabase.addlog('get task detail data from server called api failed' + JSON.stringify(requestTaskModel));
      //   console.log(error && error.response);
    ...
    udatabase.addlog('task detail offline data api calling with task ids' + JSON.stringify(requestTaskModel));
    baseAPI.post('Task/GetTaskDetailsJSONForOffline/', requestTaskModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async getDynamicTaskDetailData(taskIds, ismsg, ontaskdetailsuccess) {
    ...
    requestTaskModel.ClientID = clientDetail.ClientID;
    if (ismsg)
      requestTaskModel.MsgIDList = taskIds;
    else
      requestTaskModel.TaskIDList = taskIds;

    //loader on----
    //this.setState({isLoading:true});
    let APIName = ismsg == true ? 'GetMessageDetailsJSONForOffline' : 'GetTaskDetailsJSONForOffline';
    ...
    udatabase.addlog('task detail offline data api calling with task ids' + JSON.stringify(requestTaskModel));
    baseAPI.post('Task/' + APIName + '/', requestTaskModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async SetActivityAction(onactionsuccess) {
    ...
    baseAPI.post('Task/SetActivityActions/', requetActivityActionModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async SaveTaskData(ontasksave, onError) {
    ...
    console.log(JSON.stringify(requestTaskSaveModel))
    ...
    const onFailure = error => {
      udatabase.addlog('save task data from server called api failed' + JSON.stringify(requestTaskSaveModel));
      console.log(error && error.response);
      //this.setState({errors: error.response.data, isLoading: false});
      onError(error)
    };

    baseAPI.post('Task/SetTask/', requestTaskSaveModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async getActivityData(onSuccessActivity) {
    ...
    const onSuccess = ({ data }) => {
      ...
      database.saveTaskCategoryToLocal(requetActivityModel.GroupAppID, data.Result.ActivityTypesJSON).then(() => {
    ...
    baseAPI.post('Task/GetActivityTypesJSON/', requetActivityModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async getTaskDLLData(ondllsuccess) {
    ...
    baseAPI.post('Task/GetTaskDDL/', requestTaskDLLModel)
      .then(onSuccess)
      .catch(onFailure);
  ...
  async getDynamicTaskListData(groupAppID, ontasksuccess) {
    ...
    //alert(JSON.stringify(clientAppData));
    let metadata = await database.getTaskMetaDataJSONAsync(requestModel.ClientAppID);
    ...
    let groupAppList = clientAppData.filter(e => e.ClientAppID == requestModel.ClientAppID)[0].GroupAppsList;
    ...
    requestModel.GroupAppsList = requestModel.MyMessage == true ? _groupAppList : requestModel.GroupAppsList;
    ...
    const onSuccess = async ({ data }) => {
      ...
      if (data.Result.DataList.length > 0) {
        await database.setupTaskAsync(requestModel.ClientAppID, 0, data.Result.MetaData, 1, 0);
        await database.setupTaskAsync(requestModel.ClientAppID, 1, data.Result.StatusChart, 0, 1);
        //alert(JSON.stringify(data.Result.MetaData));
        data.Result.DataList.forEach(element => {

          database.setupTaskAsync(requestModel.ClientAppID, element.RowID, element.RowJSON, 0, 0);
          var rowJSON = JSON.parse(element.RowJSON);
          if (data.Result.MetaData.ProcessDelete == 'True' && rowJSON.DeleteRowID == 1) {
        ...
        console.log('task loaded and detail calling');
        ...
        //this.getDynamicTaskData(taskids);
    ...
    const onFailure = error => {
      ...
      //alert(JSON.stringify(error.response));
      console.log(error && error.response);
      udatabase.addlog('save task data from server called api failed' + JSON.stringify(requestModel));
