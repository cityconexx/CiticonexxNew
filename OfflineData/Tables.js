  ...
  return new Promise((resolve, reject) => {
    db.transaction(
      (tx) => {
        ...
        // tx.executeSql(
        //   "create table if not exists tasks (id integer primary key not null, rowid integer, clientAppId integer, ismetadata integer, isstatus integer, json JSON, taskDetail JSON); "
        // );
        tx.executeSql(
        ...
        tx.executeSql(
          "CREATE INDEX tasks_idx1 ON tasks(id, rowid, clientAppId); "
        );
        tx.executeSql(
        ...
        tx.executeSql(
          "CREATE INDEX taskDetail_idx1 ON taskDetail(id, RowId, clientAppId); "
        );
        tx.executeSql(
        ...
        tx.executeSql(
          "create table if not exists tabletaskCategory (id integer primary key not null, group_id integer, server_id integer, category_text text, items JSON); "
        );
      },
      (_, error) => {
        console.log("db error creating tables3333");
        resolve(error);
        console.log(error);
        reject(error);
      },
